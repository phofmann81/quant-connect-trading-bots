...
class TrailingStopRiskManagementModel(RiskManagementModel):
    ...
    def manage_risk(self, algorithm, targets):
        ...
        for kvp in algorithm.securities:
            ...
            position = (
                PositionSide.LONG if security.holdings.is_long else PositionSide.SHORT
            )
            absolute_holdings_value = security.holdings.absolute_holdings_value
            trailing_absolute_holdings_state = (
                self.trailing_absolute_holdings_state.get(symbol)
            )

            # Add newly invested security (if doesn't exist) or reset holdings state (if position changed)
            if (
                trailing_absolute_holdings_state == None
                or position != trailing_absolute_holdings_state.position
            ):
                self.trailing_absolute_holdings_state[symbol] = (
                    trailing_absolute_holdings_state
                ) = self.HoldingsState(
                    position, security.holdings.absolute_holdings_cost
                )

            trailing_absolute_holdings_value = (
                trailing_absolute_holdings_state.absolute_holdings_value
            )

            # Check for new max (for long position) or min (for short position) absolute holdings value
            if (
                position == PositionSide.LONG
                and trailing_absolute_holdings_value < absolute_holdings_value
            ) or (
                position == PositionSide.SHORT
                and trailing_absolute_holdings_value > absolute_holdings_value
            ):
                self.trailing_absolute_holdings_state[
                    symbol
                ].absolute_holdings_value = absolute_holdings_value
                continue

            drawdown = abs(
                (trailing_absolute_holdings_value - absolute_holdings_value)
                / trailing_absolute_holdings_value
            )
