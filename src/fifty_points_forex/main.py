...
class OcODevisenStrategy(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.currency_pairs: List[QuantConnect.Securities.Forex.Forex] = []

        for currency_pair in ["EURUSD", "GBPUSD", "EURGBP"]:
            self.currency_pairs.append(self.add_forex(ticker=currency_pair))

        self.universe_settings.resolution = Resolution.MINUTE

        self.add_universe_selection(
            ManualUniverseSelectionModel(
                [currency_pair.symbol for currency_pair in self.currency_pairs]
            )
        )
